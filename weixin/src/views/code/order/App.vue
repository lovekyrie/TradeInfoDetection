...
<template>
  <div id="container">
    ...
    <div class="pdfList" v-if="pdf.length>0">
      <iframe
        :src="'/wechat/static/pdf/web/viewer.html?file=' + item"
        height="560"
...
export default {
  ...
  methods: {
    getOrdType() {
      this.until.get("/general/cat/listByPrntCd?prntCd=80020").then(res => {
        if (res.status == "200") {
    ...
    getInfo() {
      this.until.getText("/prodx/mxordete/infoByNo/" + this.no).then(res => {
